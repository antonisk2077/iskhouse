...
function NewGuid() {
	...
	if(isset($_SESSION['objLogin'])){
		if(isset($_POST['token']) && $_POST['token'] == 'getunitinfo'){
			...
			if(isset($_POST['cid']) && !empty($_POST['cid'])){
			    $URL_IMAGE = "";
			    if(isset($_POST['image_after_work']) && !empty($_POST['image_after_work'])){
			    ...
			    if(isset($_POST['_txtSolutionList']) && !empty($_POST['_txtSolutionList'])){
			        $_txtSolutionList = htmlentities($_POST['_txtSolutionList'], ENT_QUOTES, 'UTF-8');
			        $sql .= " , solution_List = '".$_txtSolutionList."' ";
			    }
				mysqli_query($link,"UPDATE tbl_add_complain SET job_status = '".$_POST['status']."', solution = '".$_POST['txt']."' $sql where complain_id = ".(int)$_POST['cid']);
			}
			echo 'success';
			die();
		}		
