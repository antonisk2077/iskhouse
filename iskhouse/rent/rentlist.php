...
$branch_filter_sql = ($filter_branch_id > 0) ? " and r.branch_id = " . $filter_branch_id : "";
        ...
        <div class="tab-content" style="margin-top:12px;">
          <div id="tenant-active" class="tab-pane fade in active">
            <table class="table sakotable table-bordered table-striped dt-responsive">
              ...
              <tbody>
                <?php
					$result = mysqli_query($link,"Select *,f.floor_no as ffloor,u.unit_no from tbl_add_rent r inner join tbl_add_floor f on f.fid = r.r_floor_no inner join tbl_add_unit u on u.uid = r.r_unit_no where r.r_status = 1".$branch_filter_sql." order by r.r_unit_no asc");
	...
	<b>Image ID :</b><?php echo !empty($row['image_id']) ? '<a target="_blank" href="'.WEB_URL.'/img/upload/'.$img_url.'" > Open Proof</a>' : ''; ?><br/>
          ...
          <div id="tenant-inactive" class="tab-pane fade">
            <table class="table sakotable table-bordered table-striped dt-responsive">
              ...
              <tbody>
                <?php
					$result = mysqli_query($link,"Select *,f.floor_no as ffloor,u.unit_no from tbl_add_rent r inner join tbl_add_floor f on f.fid = r.r_floor_no inner join tbl_add_unit u on u.uid = r.r_unit_no where r.r_status = 0".$branch_filter_sql." order by r.r_unit_no asc");
					while($row = mysqli_fetch_array($result)){
						$image = WEB_URL . 'img/no_image.jpg';	
						if(file_exists(ROOT_PATH . '/img/upload/' . $row['image']) && $row['image'] != ''){
							$image = WEB_URL . 'img/upload/' . $row['image'];
