...
$getMonth = date('F');
$year = date('Y');
$SelectMonth = mysqli_query($link,"SELECT * FROM tbl_add_month_setup WHERE month_name = '$getMonth'  LIMIT 1");
$MonthID = mysqli_fetch_assoc($SelectMonth);
$MonthID = $MonthID['m_id'];

//fair count
$result_fair = mysqli_query($link,"SELECT sum(rent) as total FROM tbl_add_fair WHERE branch_id = '".(int)$_SESSION['objLogin']['branch_id']."' AND month_id = '$MonthID' and xyear = '$year' ");
...
<section class="content">
  ...
  <div class="row">
    <div class="col-lg-12 col-xs-12">
      <div class="box box-success">
        <div class="box-header with-border">
          <h3 class="box-title"><i class="fa fa-money" aria-hidden="true"></i>&nbsp;&nbsp;<?php echo $_data['rent_due_table_title']; ?></h3>
        ...
        <div class="box-body">
          <table class="table sakotable table-bordered table-striped dt-responsive">
            <thead>
              <tr>
                <th><?php echo $_data['rent_due_col_invoice']; ?></th>
                <th><?php echo $_data['rent_due_col_tenant']; ?></th>
                <th><?php echo $_data['rent_due_col_unit']; ?></th>
                <th><?php echo $_data['rent_due_col_month']; ?></th>
                <th><?php echo $_data['rent_due_col_year']; ?></th>
                <th><?php echo $_data['rent_due_col_total']; ?></th>
            ...
            <tbody>
              <?php
				$filter_branch_id = isset($_GET['branch_id']) ? (int)$_GET['branch_id'] : (int)$_SESSION['objLogin']['branch_id'];
				$branch_due_sql = ($filter_branch_id > 0) ? " and f.branch_id = " . $filter_branch_id : "";
				$result_due = mysqli_query($link,"Select f.f_id,ar.r_name,u.unit_no,m.month_name,f.xyear,f.total_rent from tbl_add_fair f inner join tbl_add_rent ar on ar.rid = f.rid inner join tbl_add_unit u on u.uid = f.unit_no inner join tbl_add_month_setup m on m.m_id = f.month_id where f.type = 'Rented' and f.bill_status = 0".$branch_due_sql." order by f.f_id desc LIMIT 5");
				if(mysqli_num_rows($result_due) > 0){
					while($row_due = mysqli_fetch_array($result_due)){?>
              <tr>
                <td><?php echo $row_due['f_id']; ?></td>
                <td><?php echo $row_due['r_name']; ?></td>
                <td><?php echo $row_due['unit_no']; ?></td>
                <td><?php echo $row_due['month_name']; ?></td>
                <td><?php echo $row_due['xyear']; ?></td>
                <td><?php echo $ams_helper->currency($localization, $row_due['total_rent']); ?></td>
                <td>
                  <a target="_blank" class="btn btn-info ams_btn_special" data-original-title="<?php echo $_data['rent_due_col_invoice'];?>" data-toggle="tooltip" href="<?php echo WEB_URL;?>fair/invoice.php?rentid=<?php echo $row_due['f_id']; ?>"><i class="fa fa-file-text-o"></i></a>
                </td>
              </tr>
              <?php } } else { ?>
              <tr>
                <td><?php echo $_data['rent_due_empty']; ?></td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
