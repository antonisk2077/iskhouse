...
$branch_filter_sql = ($filter_branch_id > 0) ? " and c.branch_id = " . $filter_branch_id : "";
...
$employee_list = array();
$employee_list_sql = "Select *,m.member_type from tbl_add_employee e inner join tbl_add_member_type m on m.member_id = e.e_designation where e.e_status = 1";
if($filter_branch_id > 0){
	$employee_list_sql .= " and e.branch_id = " . $filter_branch_id;
}
$employee_list_sql .= " order by e.e_name ASC";
$result_emp = mysqli_query($link,$employee_list_sql);
    ...
    <div class="box box-success">
      ...
      <div class="box-body">
        <ul class="nav nav-tabs" role="tablist" style="margin-bottom:15px;">
          <li role="presentation" class="<?php echo ($status === 'progress') ? 'active' : ''; ?>"><a href="<?php echo WEB_URL; ?>complain/complainlist.php?status=progress&branch_id=<?php echo $filter_branch_id; ?>"><?php echo $_data['t_status_in_progress'];?></a></li>
          <li role="presentation" class="<?php echo ($status === 'completed') ? 'active' : ''; ?>"><a href="<?php echo WEB_URL; ?>complain/complainlist.php?status=completed&branch_id=<?php echo $filter_branch_id; ?>"><?php echo $_data['t_status_completed'];?></a></li>
        </ul>
        <table class="table sakotable table-bordered table-striped dt-responsive">
          ...
          <tbody>
            <?php
				$result = mysqli_query($link,"Select c.*,m.month_name,u.unit_no from tbl_add_complain c inner join tbl_add_month_setup m on m.m_id = c.c_month left join tbl_add_unit u on u.uid = c.c_unit_no where 1=1" . $branch_filter_sql . $status_sql . " order by complain_id DESC");
            ...
            <tr>
			  ...
			  <td>
			      <span style="display:none;"><?php echo $row['job_status']; ?></span>
						...
						?>
						  <div class="row" align="center">
						      ...
						      <?php if(!empty($image_before_sol)){ ?>
    						      <h3 style="text-decoration:underline;">Image Before Work :</h3>
                                    <a target="_blank" href="<?php echo !empty($image_before_sol) ? $image_before_sol : 'https://hearhear.org/wp-content/uploads/2019/09/no-image-icon.png'; ?>">
                                        <img class="form-control output_<?php echo $row['complain_id']; ?>" src="<?php echo !empty($image_before_sol) ? $image_before_sol : 'https://hearhear.org/wp-content/uploads/2019/09/no-image-icon.png' ; ?>" style="height:100px;width:100px;"/>
                                    </a>
                                <?php } ?>  
                                </div>  
						      ...
						      <?php if(!empty($image_after_work)){ ?>
    						      <h3 style="text-decoration:underline;">Image After Work :</h3>
                                    <a target="_blank" href="<?php echo !empty($image_after_work) ? $image_after_work : 'https://hearhear.org/wp-content/uploads/2019/09/no-image-icon.png'; ?>">
                                        <img class="form-control output_<?php echo $row['complain_id']; ?>" src="<?php echo !empty($image_after_work) ? $image_after_work : 'https://hearhear.org/wp-content/uploads/2019/09/no-image-icon.png' ; ?>" style="height:100px;width:100px;"/>
                                    </a>
                                <?php } ?>  
                                </div>  
                            </div>            
							...
							<div><?php echo $row['solution']; ?></div>
						  	<h3 style="text-decoration:underline;">Description List</h3>
							<div><?php 
